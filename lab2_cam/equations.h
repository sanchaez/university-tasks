...
#include <functional>
namespace eq {
template <class FuncType> class Equation {
public:
  Equation() = delete;
  explicit Equation(FuncType func_default, FuncType derivative_default,
  ...
  void set_range(const double &a, const double &b) {
    _range_a = a;
    _range_b = b;
  }
  ...
  double iterative(const double &precision) {
    ...
    std::function<double(double)> phi_function(
        [&](double x) { return (x - lambda_constant * _func(x)); });

    const double precision_constant =
        abs((1 - q_constant) / q_constant * precision);
  ...
  double hordes(const double &precision) {
    std::function<double(double, double)> iteration_function(
        [&](double x, double c) {
          return (x - (_func(x) * (x - c)) / (_func(x) - _func(c)));
        });

    double x0 = _range_b, xn = iteration_function(_range_b, _range_a);
...
private:
  double min_derivative_range() {
    double min_a = _derivative(_range_a), min_b = _derivative(_range_b);

    return ((min_a <= min_b) ? min_a : min_b);
  }

  double max_derivative_range() {
    double max_a = _derivative(_range_a), max_b = _derivative(_range_b);

    return ((max_a >= max_b) ? max_a : max_b);
  }

  double lambda_factor() { return (1 / max_derivative_range()); };
